...
// 画布节点系统已移除，移除 activeWorkflowId/switchWorkflow 引用，禁用工作流切换交互
...
/**
 * 工作流列表面板
 * 画布节点系统已移除，工作流切换功能暂不可用，保留 UI 结构但禁用交互
 */
export const WorkflowList: React.FC = () => {
  ...
  /** 工作流切换（画布节点系统已移除，暂为空操作） */
  const handleActivate = (_templateId: string, _nodes: any[], _edges: any[]) => {
    AppNotifier.info('工作流切换功能开发中');
  };
  ...
  return (
    <div className="flex flex-col h-full bg-background p-3 gap-2.5 overflow-y-auto custom-scrollbar select-none relative">
      ...
      {/* 当前项目入口 */}
      <div
        className="relative flex items-center gap-3 p-3 px-4 rounded-lg transition-all cursor-default outline-none bg-zinc-900 border border-zinc-700"
      >
