 ...
 * - PRECISE_SINGLE：精准单帧时间戳截图
 */
export function buildExtractCommand(config: ExtractConfig): string[] {
  const {
    videoPath, outputPath, strategy = 'VLM_OPTIMIZED',
    fps = 1, sceneThreshold = 0.28, minFrameInterval = 4,
    width = 1024, quality = 3, inPoint, outPoint,
    timePoint, threads = 2,
  } = config;
  ...
  switch (strategy) {
    ...
    case 'PRECISE_SINGLE': {
      /** 单帧截图不需要 select/fps 滤镜 */
      break;
    }
  }

  /** 缩放：VLM 模式推荐 1024，其他模式默认 1024 */
