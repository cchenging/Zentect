...
// === Mock 外部依赖（resolveStrategy 和 extractFrames 均不依赖 infra） ===

const { mockGetFfmpegPath, mockGetFfprobePath, mockExistsSync, mockReaddirSync, mockMkdirSync, mockUnlinkSync, mockStat, mockPromisesStat, mockSpawn } = vi.hoisted(() => ({
...
describe('FrameExtractionService', () => {
  ...
  describe('extractFrames', () => {
    ...
    it('不存在的输出目录应自动创建', async () => {
      mockMkdirSync.mockImplementation(() => undefined);
      const mkdirSyncSpy = mockMkdirSync;
      ...
      const mkdirCalls = mkdirSyncSpy.mock.calls.filter(
    ...
    it('特殊字符 mediaId 应被安全替换', async () => {
      ...
      mockSpawn.mockImplementation(() => {
        ...
        const proc = createMockSpawn({ closeCode: 0 }) as any;
        // check the output path argument contains safe characters
        return proc;
      });
    ...
    it('远程测应正确统计文件数量和大小', async () => {
      mockExistsSync.mockReturnValue(true);
      mockMkdirSync.mockImplementation(() => undefined);
      mockReaddirSync
        .mockReturnValueOnce([]) // 旧帧清理
        .mockReturnValueOnce(['frame_00000001.jpg', 'frame_00000002.jpg']); // 结果
