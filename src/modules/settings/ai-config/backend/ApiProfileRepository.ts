...
function rowToProfile(row: RawRow): ApiProfile {
  return {
    ...
    apiKey: row.api_key ? credentialManager.decrypt(row.api_key) : '',
...
export class ApiProfileRepository {
  ...
  static update(id: string, patch: Partial<ApiProfile>): boolean {
    ...
    if (patch.apiKey !== undefined) { sets.push('api_key = ?'); vals.push(patch.apiKey ? credentialManager.encrypt(patch.apiKey) : null); }
