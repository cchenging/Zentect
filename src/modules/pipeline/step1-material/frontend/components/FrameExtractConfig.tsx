      ...
      <div className="rounded-lg border border-border/50 bg-muted/20 p-2.5 shadow-inner">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[12px] font-mono text-accent-green font-semibold bg-accent-green/10 px-1.5 rounded">{sceneThreshold}</span>
                </div>
                <Slider min={0.1} max={0.5} step={0.01} value={[sceneThreshold]} onValueChange={([v]) => updateFrames({ sceneThreshold: v })} disabled={isRunning} />
              </div>
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[12px] text-muted-foreground">最小间隔</span>
                  <span className="text-[12px] font-mono text-accent-cyan font-semibold bg-accent-cyan/10 px-1.5 rounded">{minInterval}s</span>
                </div>
        ...
        {strategy === 'UNIFORM_FPS' && (
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <span className="text-[12px] text-muted-foreground">抽取频率</span>
              <span className="text-[12px] font-mono text-accent-cyan font-semibold bg-accent-cyan/10 px-1.5 rounded">{fps} 帧/秒</span>
            </div>
            <Slider min={0.1} max={5} step={0.1} value={[fps]} onValueChange={([v]) => { updateFrames({ fps: v }); }} disabled={isRunning} />
            <div className="flex justify-between text-[11px] text-muted-foreground/50 px-0.5">
        ...
        {strategy === 'FAST_KEYFRAME' && (
          <p className="text-[12px] text-muted-foreground/80 leading-relaxed text-center py-1">
        ...
        {strategy === 'PRECISE_SINGLE' && (
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <span className="text-[12px] text-muted-foreground">截图时间点</span>
              <span className="text-[12px] font-mono text-foreground">{(frames as any).timePoint ?? 0}s</span>
            </div>
            <Input
        ...
        <div className="grid grid-cols-2 gap-1.5">
          ...
          <div className="rounded-lg border border-border/50 bg-muted/20 p-2 flex flex-col gap-1.5 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[12px] font-mono text-accent-warm font-semibold">{quality}/5</span>
            </div>
