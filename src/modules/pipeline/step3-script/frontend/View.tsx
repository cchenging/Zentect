...
import { SCRIPT_STYLES } from "../../../../renderer/src/pages/editor/utils/pipelineConstants";
...
export const StepScriptGenerationView: React.FC<StepScriptGenerationProps> = (props) => {
  const {
    scriptParagraphs, scriptStyle, speechRate, pipelineParams, vlmFrames, isGenerating,
    onSetScriptStyle, onSetSpeechRate, onSetPipelineParams, onUpdateParagraph,
    onUpdateParagraphEmotion, onRegenerate, onMatchVision,
  } = props;
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        {scriptParagraphs.length > 0 && (
          <StatHeader value={scriptParagraphs.length} unit="段文案" secondary={`预估 ${Math.ceil(scriptParagraphs.reduce((s, p) => s + (p.duration || 0), 0) / 60)} 分钟`} />
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <select
          value={scriptStyle}
          onChange={(e) => onSetScriptStyle(e.target.value)}
              ...
              <div key={pId} className="glass-card-sm p-3">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <div className="flex items-center gap-1 ml-auto">
                    ...
                    <select
                      value={p.emotion || ""}
                      onChange={(e) => onUpdateParagraphEmotion(p.id, e.target.value)}
