...
export class MigrationManager {
  ...
  constructor(db: Database.Database, migrationsDir?: string) {
    ...
    if (migrationsDir) {
      ...
      const candidates = [
        path.join(__dirname, 'migrations'),                                    // 打包态: out/main/database/migrations/
        path.join(appRoot, 'src', 'main', 'database', 'migrations'),           // 开发态: 从 appRoot 定位
        path.join(process.cwd(), 'src', 'main', 'database', 'migrations'),     // cwd 回退
        path.join(__dirname, '..', '..', '..', 'src', 'main', 'database', 'migrations'), // 相对回退
      ];
   ...
   * SQLite 不支持 IF NOT EXISTS on ALTER TABLE，因此需要程序化检查
   * 此方法将 004 迁移文件的逻辑以安全方式执行
   */
  safeAddColumns(): void {
