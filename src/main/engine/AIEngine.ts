...
export class AIEngine {
  ...
  public static async streamChat(webContents: WebContents, taskType: AITaskType, systemPrompt: string, userMessage: string, replyChannel: string) {
    try {
      // 👑 严格执宪：统一走 LLMFactory.createAdapter 单一配置源（模型映射绑定表优先）
      const { adapter, modelName, temperature } = LLMFactory.createAdapter(taskType);

      webContents.send(`${replyChannel}:start`, { model: modelName, task: taskType });
      ...
      await adapter.streamChatToBrowser(webContents, messages, modelName, temperature, replyChannel);
  ...
  static async analyzeVision(imagePath: string): Promise<string> {
    const config = LLMFactory.getEffectiveConfig('visual');
  ...
  // ---------------------------------------------------------------------------
  public static async agentStreamChat(webContents: WebContents, projectId: string, prompt: string, context: any, history: any[] = []) {
    ...
    try {
      ...
      // 统一走 LLMFactory.createAdapter('helper') 单一配置源（模型映射绑定表优先）
      const { adapter, modelName, temperature } = LLMFactory.createAdapter('helper');
      ...
      const result = await adapter.streamChatToBrowser(webContents, messages, modelName, temperature, IPC_CHANNELS.AGENT_STREAM_CHUNK, tools);
