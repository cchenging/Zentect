...
export const FaceDetectionSubStep: React.FC<FaceDetectionSubStepProps> = ({ expanded, onToggle }) => {
  ...
  return (
    <CollapsibleCard expanded={expanded} onExpandedChange={onToggle}
      title={<><StatusIcon status={status === 'idle' ? 'pending' : status} /><span className="text-[12px] font-semibold">人物识别</span></>}
      extra={<span className="text-[11px] text-muted-foreground">{status === 'completed' ? roles.length + ' 个角色' : '等待'}</span>}>
        ...
        <div className="grid grid-cols-3 gap-2">
          {roles.map((role: any) => (
            <div key={role.id} className="flex flex-col items-center gap-1 p-2 rounded-md bg-bg-secondary">
              ...
              <span className="text-[11px]">{role.name || '未命名'}</span>
            </div>
          ))}
        </div>
      )}
